...
export class Drag extends DragHelper {
  ...
  override createProxy = (grabbedElement: HTMLElement): HTMLDivElement => {
    ...
    Object.assign(proxy.style, {
      ...
      height: `${schedule.rowHeight - 2 * (schedule.resourceMargin as number)}px`,
    });
    ...
    let totalDuration = 0;
    (grid.selectedRecords as Appointment[]).forEach(
  ...
  override onDragStart = ({ context }: { context: any }): void => {
    ...
    context.relatedElements = selectedRecords
      .sort(
        (r1, r2) => (store as Store).indexOf(r1) - (store as Store).indexOf(r2),
      )
